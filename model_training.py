...
class GoldTradingModel:
    ...
    def train(
        ...
        logger.info(
            f"[STATS]  Paramètres: n_estimators={params.get('n_estimators')}, max_depth={params.get('max_depth')}, lr={params.get('learning_rate')}"
        )
        ...
        if X_val is not None and y_val is not None and early_stopping:
            ...
            self.model.fit(
                X_train, y_train, eval_set=[(X_train, y_train)], verbose=False
            )
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray) -> dict:
        ...
        print(
            classification_report(
                y_test, y_pred, target_names=["VENTE (0)", "ACHAT (1)"]
            )
        )
    ...
    def plot_confusion_matrix(
        self, y_test: np.ndarray, y_pred: np.ndarray, save_path: str = None
    ):
        """
    ...
    def plot_roc_curve(
        self, y_test: np.ndarray, y_pred_proba: np.ndarray, save_path: str = None
    ):
        """
