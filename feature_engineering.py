...
class FeatureEngineer:
    ...
    def add_price_features(self, prefix: str = "gold"):
        ...
        # Log returns
        self.df["log_return_1d"] = np.log(
            self.df[close_col] / self.df[close_col].shift(1)
        )
        self.df["log_return_5d"] = np.log(
            self.df[close_col] / self.df[close_col].shift(5)
        )

        # Volatilité historique (rolling std des returns)
        for period in [5, 10, 20, 30, 60]:
            self.df[f"volatility_{period}d"] = (
                self.df["return_1d"].rolling(period).std()
            )
            self.df[f"volatility_{period}d_ann"] = self.df[
                f"volatility_{period}d"
            ] * np.sqrt(252)

        # Range du jour (High-Low)
        if high_col in self.df.columns and low_col in self.df.columns:
            self.df["daily_range"] = (self.df[high_col] - self.df[low_col]) / self.df[
                close_col
            ]
            ...
            for period in [7, 14, 21]:
                ...
                self.df[f"atr_{period}_pct"] = (
                    self.df[f"atr_{period}"] / self.df[close_col]
                )

        # Gap d'ouverture
        if open_col in self.df.columns:
            self.df["gap"] = (
                self.df[open_col] - self.df[close_col].shift(1)
            ) / self.df[close_col].shift(1)
            self.df["gap_ma5"] = self.df["gap"].rolling(5).mean()

        # Distance aux extremes
        for period in [20, 50, 100, 200]:
            self.df[f"dist_high_{period}d"] = (
                self.df[close_col] - self.df[close_col].rolling(period).max()
            ) / self.df[close_col]
            self.df[f"dist_low_{period}d"] = (
                self.df[close_col] - self.df[close_col].rolling(period).min()
            ) / self.df[close_col]

        # Momentum à différentes échelles
        for period in [5, 10, 20, 60]:
            self.df[f"momentum_{period}d"] = self.df[close_col] - self.df[
                close_col
            ].shift(period)
    ...
    def add_moving_averages(self, prefix: str = "gold"):
        ...
        for period in ma_periods:
            ...
            self.df[f"ema_{period}"] = (
                self.df[close_col].ewm(span=period, adjust=False).mean()
            )

            # Distance au prix (en %)
            self.df[f"dist_sma_{period}"] = (
                self.df[close_col] - self.df[f"sma_{period}"]
            ) / self.df[f"sma_{period}"]
            self.df[f"dist_ema_{period}"] = (
                self.df[close_col] - self.df[f"ema_{period}"]
            ) / self.df[f"ema_{period}"]
        ...
        for fast, slow in ma_pairs:
            if f"sma_{fast}" in self.df.columns and f"sma_{slow}" in self.df.columns:
                self.df[f"ma_cross_{fast}_{slow}"] = (
                    self.df[f"sma_{fast}"] > self.df[f"sma_{slow}"]
                ).astype(int)
                # Signal de croisement
                self.df[f"ma_cross_signal_{fast}_{slow}"] = self.df[
        ...
        for period in [20, 50, 200]:
            if f"sma_{period}" in self.df.columns:
                self.df[f"sma_{period}_slope"] = self.df[f"sma_{period}"].diff(
                    5
                ) / self.df[f"sma_{period}"].shift(5)
    ...
    def add_rsi(self, prefix: str = "gold"):
        ...
        for period in rsi_periods:
            ...
            self.df[f"rsi_{period}_overbought"] = (
                self.df[f"rsi_{period}"] > 70
            ).astype(int)
            self.df[f"rsi_{period}_oversold"] = (self.df[f"rsi_{period}"] < 30).astype(
                int
            )
            self.df[f"rsi_{period}_extreme_ob"] = (
                self.df[f"rsi_{period}"] > 80
            ).astype(int)
            self.df[f"rsi_{period}_extreme_os"] = (
                self.df[f"rsi_{period}"] < 20
            ).astype(int)
    ...
    def add_macd(self, prefix: str = "gold"):
        ...
        self.df["macd_hist_increasing"] = (
            self.df["macd_histogram"] > self.df["macd_histogram"].shift(1)
        ).astype(int)
        ...
        self.df["macd_swing_signal"] = (
            self.df["macd_swing"].ewm(span=5, adjust=False).mean()
        )
    ...
    def add_bollinger_bands(self, prefix: str = "gold"):
        ...
        for period in [10, 20, 50]:
            for std_mult in [1.5, 2, 2.5]:
                ...
                self.df[f"bb_width_{suffix}"] = (
                    self.df[f"bb_upper_{suffix}"] - self.df[f"bb_lower_{suffix}"]
                ) / sma

                # Position du prix dans les bandes (0 = bas, 1 = haut)
                self.df[f"bb_position_{suffix}"] = (
                    self.df[close_col] - self.df[f"bb_lower_{suffix}"]
                ) / (self.df[f"bb_upper_{suffix}"] - self.df[f"bb_lower_{suffix}"])
        ...
        self.df["bb_width"] = (self.df["bb_upper"] - self.df["bb_lower"]) / self.df[
            "bb_middle"
        ]
        self.df["bb_position"] = (self.df[close_col] - self.df["bb_lower"]) / (
            self.df["bb_upper"] - self.df["bb_lower"]
        )

        # Signaux de breakout
        self.df["bb_breakout_up"] = (self.df[close_col] > self.df["bb_upper"]).astype(
            int
        )
        self.df["bb_breakout_down"] = (self.df[close_col] < self.df["bb_lower"]).astype(
            int
        )

        # Squeeze (faible volatilité)
        self.df["bb_squeeze"] = (
            self.df["bb_width"] < self.df["bb_width"].rolling(50).mean() * 0.75
        ).astype(int)
    ...
    def add_momentum_indicators(self, prefix: str = "gold"):
        ...
        for period in [5, 10, 14, 20, 30, 60]:
            self.df[f"roc_{period}"] = (
                (self.df[close_col] - self.df[close_col].shift(period))
                / self.df[close_col].shift(period)
                * 100
            )
        ...
        if high_col in self.df.columns and low_col in self.df.columns:
            for period in [5, 9, 14, 21]:
                ...
                self.df[f"stoch_k_{period}"] = (
                    100 * (self.df[close_col] - low_min) / (high_max - low_min)
                )
                self.df[f"stoch_d_{period}"] = (
                    self.df[f"stoch_k_{period}"].rolling(3).mean()
                )
        ...
        if high_col in self.df.columns and low_col in self.df.columns:
            for period in [10, 14, 20]:
                ...
                self.df[f"williams_r_{period}"] = (
                    -100 * (high_max - self.df[close_col]) / (high_max - low_min)
                )
        ...
        if high_col in self.df.columns and low_col in self.df.columns:
            ...
            tr = (
                self.df["true_range"]
                if "true_range" in self.df.columns
                else (self.df[high_col] - self.df[low_col])
            )
        ...
        # Ultimate Oscillator
        if high_col in self.df.columns and low_col in self.df.columns:
            bp = self.df[close_col] - np.minimum(
                self.df[low_col], self.df[close_col].shift(1)
            )
            tr = np.maximum(
                self.df[high_col], self.df[close_col].shift(1)
            ) - np.minimum(self.df[low_col], self.df[close_col].shift(1))
    ...
    def add_volume_indicators(self, prefix: str = "gold"):
        ...
        if high_col in self.df.columns and low_col in self.df.columns:
            clv = (
                ...
                - (self.df[high_col] - self.df[close_col])
            ) / (self.df[high_col] - self.df[low_col])
            clv = clv.fillna(0)
    ...
    def add_macro_features(self):
        ...
        if "dxy_close" in self.df.columns and gold_close in self.df.columns:
            ...
            for period in [20, 60]:
                self.df[f"gold_dxy_corr_{period}d"] = (
                    self.df[gold_close]
                    .pct_change()
                    .rolling(period)
        ...
        if "vix_close" in self.df.columns:
            ...
            if gold_close in self.df.columns:
                for period in [20, 60]:
                    self.df[f"gold_vix_corr_{period}d"] = (
                        self.df[gold_close]
                        .pct_change()
                        .rolling(period)
        ...
        if "us10y_close" in self.df.columns:
            ...
            self.df["us10y_above_ma"] = (
                self.df["us10y_close"] > self.df["us10y_ma_20"]
            ).astype(int)
        ...
        if "silver_close" in self.df.columns and gold_close in self.df.columns:
            ...
            self.df["gold_silver_ratio_ma20"] = (
                self.df["gold_silver_ratio"].rolling(20).mean()
            )
            self.df["gold_silver_ratio_high"] = (
                self.df["gold_silver_ratio"] > self.df["gold_silver_ratio_ma20"]
            ).astype(int)
        ...
        if "platinum_close" in self.df.columns and gold_close in self.df.columns:
            self.df["gold_platinum_ratio"] = (
                self.df[gold_close] / self.df["platinum_close"]
            )
        ...
        if "copper_close" in self.df.columns:
            ...
            if gold_close in self.df.columns:
                self.df["gold_copper_ratio"] = (
                    self.df[gold_close] / self.df["copper_close"]
                )
        ...
        if "oil_close" in self.df.columns:
            ...
            self.df["oil_above_ma50"] = (
                self.df["oil_close"] > self.df["oil_ma_50"]
            ).astype(int)
        ...
        if "sp500_close" in self.df.columns:
            ...
            self.df["sp500_above_ma200"] = (
                self.df["sp500_close"] > self.df["sp500_ma_200"]
            ).astype(int)

            if gold_close in self.df.columns:
                for period in [20, 60]:
                    self.df[f"gold_sp500_corr_{period}d"] = (
                        self.df[gold_close]
                        .pct_change()
                        .rolling(period)
        ...
        if "gold_miners_close" in self.df.columns and gold_close in self.df.columns:
            self.df["gdx_gold_ratio"] = (
                self.df["gold_miners_close"] / self.df[gold_close] * 100
            )
            self.df["gdx_return_5d"] = self.df["gold_miners_close"].pct_change(5)
            self.df["gdx_outperform"] = (
                self.df["gdx_return_5d"] > self.df["return_5d"]
            ).astype(int)
    ...
    def add_temporal_features(self):
        ...
        self.df["gold_season_strong"] = (
            self.df["month"].isin([1, 2, 8, 9, 11, 12]).astype(int)
        )
    ...
    def add_pattern_features(self, prefix: str = "gold"):
        ...
        self.df["streak_up"] = (
            self.df["consecutive_up"]
            .groupby(
                (
                    self.df["consecutive_up"] != self.df["consecutive_up"].shift()
                ).cumsum()
            )
            .cumsum()
        )
        self.df["streak_down"] = (
            ...
            .groupby(
                (
                    self.df["consecutive_down"] != self.df["consecutive_down"].shift()
                ).cumsum()
            )
            .cumsum()
        )
        ...
        if all(col in self.df.columns for col in [open_col, high_col, low_col]):
            ...
            # Hammer
            upper_shadow = self.df[high_col] - np.maximum(
                self.df[close_col], self.df[open_col]
            )
            lower_shadow = (
                np.minimum(self.df[close_col], self.df[open_col]) - self.df[low_col]
            )
            self.df["is_hammer"] = (
                (lower_shadow > 2 * body)
            ...
            self.df["bullish_engulfing"] = (
                (self.df[close_col].shift(1) < self.df[open_col].shift(1))
                & (self.df[close_col] > self.df[open_col])
        ...
        if high_col in self.df.columns and low_col in self.df.columns:
            self.df["higher_high"] = (
                self.df[high_col] > self.df[high_col].shift(1)
            ).astype(int)
            self.df["lower_low"] = (
                self.df[low_col] < self.df[low_col].shift(1)
            ).astype(int)
            self.df["higher_low"] = (
                self.df[low_col] > self.df[low_col].shift(1)
            ).astype(int)
    ...
    def build_all_features(self, prefix: str = "gold") -> pd.DataFrame:
        ...
        logger.info("=" * 60)
        logger.info(
            f"[STATS]  Lignes: {initial_len:,} → {final_len:,} (supprimées: {initial_len - final_len:,})"
        )
        logger.info(f"[CHART]  Features créées: {len(feature_cols)}")

        if len(self.df) > 0:
            logger.info(
                f"[DATE]  Période: {self.df.index.min().strftime('%Y-%m-%d')} → {self.df.index.max().strftime('%Y-%m-%d')}"
            )
        else:
    ...
    def save_processed_data(self, filepath: str = None):
        ...
        logger.info(
            f"[SAVED]  Données traitées sauvegardées: {filepath} ({size_mb:.2f} MB)"
        )
