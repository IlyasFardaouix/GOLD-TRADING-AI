...
class MarketDataCollector:
    ...
    def __init__(
        self, symbols: dict = None, start_date: str = None, end_date: str = None
    ):
    ...
    def fetch_single_asset(
        self, symbol: str, name: str, retries: int = 3
    ) -> pd.DataFrame:
        """
        ...
        for attempt in range(retries):
            try:
                logger.info(
                    f"[DOWNLOAD] Telechargement {name} ({symbol})... [Tentative {attempt + 1}]"
                )
                ...
                logger.info(
                    f"[OK] {name}: {len(df):,} lignes ({df.index.min().strftime('%Y-%m-%d')} -> {df.index.max().strftime('%Y-%m-%d')})"
                )
                return df
    ...
    def fetch_all_data(self) -> pd.DataFrame:
        ...
        logger.info(
            f"[DATE] Periode: {merged_df.index.min().strftime('%Y-%m-%d')} -> {merged_df.index.max().strftime('%Y-%m-%d')}"
        )
        logger.info(
            f"[DURATION] Duree: {(merged_df.index.max() - merged_df.index.min()).days:,} jours"
        )
    ...
    def get_intraday_data(
        self, symbol: str = "GC=F", interval: str = "1h", period: str = "5d"
    ) -> pd.DataFrame:
        """
